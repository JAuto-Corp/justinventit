...
#
# Restore: hub-snapshot.sh import @"$HUB_ENV" <snapshot-dir> --replace
#          hub-snapshot.sh verify @"$HUB_ENV" <snapshot-dir>
set -euo pipefail
...
grep -q '^HUB_DB_URL=' "$HUB_ENV" || { echo "hub-backup: HUB_DB_URL missing from $HUB_ENV" >&2; exit 2; }

umask 077
mkdir -p "$DEST"
snap="$DEST/$(date -u +%Y%m%dT%H%M%SZ)"
"$HERE/hub-snapshot.sh" export "@$HUB_ENV" "$snap" >/dev/null   # @file: the password never reaches argv
echo "hub-backup: $snap ($(du -sh "$snap" | cut -f1) on disk)"
