...
Base: `main` at 08c811d after PR #51 (PRs #47–#53, each merged at an independently reviewed exact head; PRs #52, #53
and #51 carry the `bookend:` field-test line — zero duplicate rounds).
...
| `route-line` | policy | R2 | `docs/MODEL_MATRIX.md` §3c, `docs/ROADMAP.md` | a review receipt, PR body or acceptance record to carry it | at most one `route:` line per sampled boundary as the only instrument of evidence-calibrated model routing; hypothesis stated, no rule change, no store/scheduler/form |
| `scope-classify` | script | R1 | `template/scripts/scope-classify.py`, `template/scripts/test_scope_classify.py`, `template/scripts/scope-classes.json.jinja` (rendered `scripts/scope-classes.json`, project-owned via `copier.yml` `_skip_if_exists`), `scripts/ci/generate-matrix-check.sh` (generated classifier test + rendered-JSON check), `docs/ROADMAP.md` | python3 ≥ 3.9; git for range mode (`--paths-from` needs none) | the scope-classifier ENGINE: prints `class=<Quick \| Quick(tooling) \| Standard+> reason=<trace>` from a git range or a path list and the authored config; implements TDD_GATE §2 rules 1–4 mechanically (triggers, count, executable class, denylist, inventory, consumer grep, product imports, test-in-change or `--red-recorded`); ships INERT — `tooling.inventory: []` — so no helper/test change can be classified `Quick(tooling)` until a project authors pairs (a whitespace-only edit of the config itself can still qualify); known v1 limit: `git grep -I` skips binary-attributed files (ROADMAP follow-up); no gate wired yet |

Known limits disclosed with this release: the `scope-classify` engine ships with the `Quick(tooling)` sub-case INERT
(empty inventory: no helper/test change can qualify; only a whitespace-only edit of the config itself can) — its consumer guarantee searches with `git grep -I`, so files `.gitattributes` marks binary are not
searched; re-scope before authoring a non-empty inventory (ROADMAP). The generated entry contract is at version 2:
projects generated at version 1 receive a non-blocking session-start warning until their AGENTS.md is updated. Check
`06-harness-sensitivity` remains specified, not built. The runtime availability receipt still requires CI's exact CLI
versions. `route:` lines are a sampled instrument, not a rule; the model matrix values are unchanged pending evidence.
`evidence-seal` still reports a repository HEAD only via `git rev-parse` and never inspects worktree state (carried from
v0.2.0).
