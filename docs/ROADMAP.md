...
- [ ] Evidence-calibrated model routing (`MODEL_MATRIX.md` §3c) — route line on existing records; manual extraction (`ROUTE-BASELINE-01.md` in W's evidence dir seeds it); dated Artificial Analysis snapshot as priors (`aa-snapshot-20260915/`); exit when the owner brief's five questions have evidence (brief: `~/.jauto-orchestration/evidence/model-routing-benchmark-handoff-r-20260915/BRIEF.md` — stable default direction per task class; provider-neutral classes; effort worth its cost per class; the observation that changes a route; optional/reversible/portable); matrix delta + reversible canary only then. Not a JA prerequisite.
- [x] Scope-classifier engine (`TDD_GATE.md` §2/§5) · `template/scripts/scope-classify.py` + `test_scope_classify.py` + stack-aware `scope-classes.json.jinja`; run per generated project by `generate-matrix-check.sh`; ships with the `tooling` sub-case INERT (empty inventory, owner scope choice 2026-09-15 after eight bounded reviews); gate wiring (check 01 / CI adapter) is a separate item
- [ ] Re-scope the `Quick(tooling)` consumer guarantee before recommending a non-empty inventory: the textual-reference search is `git grep -I` (binary-attributed files unseen); decide "every tracked file regardless of attributes" vs a narrower promise, one exact-head review, then a canary — never another resolver expansion (DECISION-scope-classify-v1-invariant.md)
...
- [x] `evidence-seal` — one provider-neutral command (`template/scripts/evidence-seal.py {seal|verify} <dir>`, python3 only, shaped like `evidence-delta.py` + its test) that REGENERATES a packet's `SHA256SUMS` (coreutils format, relative paths, `LC_ALL=C` order, optional declared `.sealignore`) and prints the manifest digest for the hub finding (plus the commit HEAD via `git rev-parse` only — never status/diff, which run content filters — when the packet lies inside a repository; packets under `~/.jauto-orchestration/evidence` do not); never appends or edits; refuses to reseal a failing manifest without `--reseal` and then prints the entry delta; `verify` is cwd-independent and exits per the check contract (0 pass · 1 changed/missing entry named · 3 unlisted files named — neither 1 nor 3 is success; no further policy switches). Acceptance: `test_evidence_seal.py` (idempotent bytes, cross-cwd verify, each exit class, no absolute path ever, output parses with `sha256sum -c`, no-repo packets seal without git) + a CI step. Compatibility: additive, optional, exact `sha256sum` format so every existing manifest/verifier keeps working; no status store; JA adoption = one file copy, no name collision. Rollback: delete script + test AND the CI step that invokes them; written manifests stay plain. Tier R1 (reversible source edit, DELIVERY.md). Field evidence + KEEP/PRUNE/AUTOMATE/DEFER: W `SEALS-PERSPECTIVE-01.md` 2026-09-14 (the PR #43 manifest incident). Built 2026-09-15 · `template/scripts/evidence-seal.py` + `test_evidence_seal.py`, run per generated project by `generate-matrix-check.sh`.
