...
models:                     # per-runtime model ids per TIER — complete on every runtime
  claude: { thinking: fable-5, authoring: fable-5, doing: opus-5-5, maint: sonnet-5 }   # 2026-07-27 correction: FABLE THINKS, OPUS DOES — opus-as-thinker caused over-engineering-through-misunderstanding; thinking seats consult Sol xhigh frequently (funnel, red-team, escalated judgment)
...
now reads `opus-5-5`. Consumers that use the Claude Code `opus` / `opus[1m]` aliases (launcher
