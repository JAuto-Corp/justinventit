...
| **Tier verification (both runtimes, mandatory)** | launcher confirms the booted session reports the intended model+effort | **empirically required** (reproduced twice, 2026-07-29): a nonexistent Codex profile boots the DEFAULT tier with exit 0, NO warning on any stream — the launcher runs a post-boot probe asserting the RESOLVED model/effort and fails loudly on mismatch; tier selection is never trusted. The resolved tier is readable ONLY from the session rollout's `turn_context` record (`~/.codex/sessions/**/rollout-*.jsonl`) — `codex doctor --json` reports `<default>` and rejects `--profile`; `codex exec --json` never emits the tier |
| **Modal-parking detection (mandatory on resume)** | the post-boot probe doubles as liveness proof — but an unanswered probe alone classifies as `unresponsive_unknown`; it upgrades to PARKED only on a modal signature (capability `modal_detectable`) or human confirmation | **booted ≠ live**: a resumed session can park on an interactive modal; while parked, its main loop takes no turns and in-session subagent deliveries BLOCK (complete but cannot land — the empty-report signature). **`modal_detectable` is claimed against a SET of signatures with a catch-all "TUI waiting on a numbered-choice prompt" fallback, never one string** — measured 2026-07-29: the parker was NOT the assumed summary-vs-verbatim prompt but an UPDATE-AVAILABLE prompt, version-triggered (appears on some launches and not others, and can start appearing with nothing in the fleet changed — a usually-absent modal trains operators to expect none) |
