...
| 2 | spec-audit | cross_review — cardinality, second-opinion, and separation rules per THE normative profile table (`ARCHITECTURE.md` §2); arms launched by the runtime-neutral review-arm tool with receipt attestation (`MODEL_MATRIX.md` §3b) | verdicts, dispositions, one hub `complete` row per arm | all blockers dispositioned; PLAUSIBLE-only findings noted; every counted arm's receipt QUALIFYING |
